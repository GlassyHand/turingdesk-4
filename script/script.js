...
if (isIncheonSpecial) {
    // 인천 출신자 중 50%는 인간, 50%는 로봇 (전체의 15%는 인간, 15%는 로봇)
...
// ✅ 최종 isHuman 판단
let isHuman;
// 로봇 외형인 경우 무조건 로봇 (최우선 체크)
if (characterData.isRobot) {
    isHuman = false;
} else if (isIncheonSpecial) {
    // 5일차 인천 특수 예외: isIncheonHuman 값에 따라 결정
    // 단, 로봇 목소리(isDialogueBlocked)나 차단된 만료기한(isBlockedExpiry)인 경우는 무조건 로봇
    isHuman = isIncheonHuman && !isDialogueBlocked && !isBlockedExpiry;
} else if (isArmImage && isArmHuman) {
    // -arm 이미지이고 50% 확률로 인간인 경우 (의수/의족)
    // 단, 로봇 목소리(isDialogueBlocked)인 경우는 무조건 로봇
...
function showFeedbackText(event, isCorrect, character) {
    ...
    // -arm 이미지이고 인간이었는데 deny한 경우
    if (character.isArmImage && character.isArmHuman && !isCorrect) {
        reasons.push('의수였음');
    }
